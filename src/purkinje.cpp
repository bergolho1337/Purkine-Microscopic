	...
	while (ptr->getIdFiber() == 1)
	{
		...
		if (r <= prob_gapJ)
		{
			...
			if (ptr2 != NULL)
			{
				...
				ptr->getCell()->sigma = ptr->getCell()->sigmaY = sigma_t;
				ptr2->getCell()->sigma = ptr->getCell()->sigmaY = sigma_t;
...
void Graph::makeRoot (double Dx)
{
	...
	c_nodes = 1;
	c_segments = 0;
	fiber_length = (rand() % 5 + 5)*scale_size;
	// Montar cada fibra 
	for (i = 0; i < fib_in_bundle; i++)
	{
		x = 0.0;
...
void Graph::calculateVelocity (double t, Node *ptr1, Node *ptr2, double Dt, double Dx, double *maxV, int exec_number)
{
	...
	if (!flag)
	{
		if (ptr1->getCell()->y[0] < ptr1->getCell()->y_new[0])
		{
	...
	{
		// Descobre o último volume da fibra 1
		ptr2 = searchNode(start_fiber[1]->getId()-1);
		...
		if (!calcVelocity && fabs(*maxV-ptr2->getCell()->y_new[0]) < 1.5)
		{
			delta_t = t-t1;
